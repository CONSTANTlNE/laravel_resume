@extends('admin.index')
@section('projects')
{{--@php--}}
{{--dd($projects)--}}
{{-- @endphp--}}
    <div class="row layout-spacing" style="padding-top: 2rem">
        <button style="width:max-content" class="btn btn-dark mb-2 me-4 " id="openModalBtn">Add Project</button>
        <!--Add New Skill  -->

        <div id="modal" class="modal">
            <div class="modal-content">
                <h2>Add New Project</h2>
                <form id="editForm" method="post" action="{{route('add_project')}}" enctype="multipart/form-data">
                    @csrf
                    <label for="name">Project Name</label>
                    <input type="text" id="name" name="project_name"/>
                    <label for="code_url">Source Code</label>
                    <input type="text" id="code_url" name="code_url"/>
                    <label for="project_url">Project Link</label>
                    <input type="text" id="code_url" name="project_url"/>
                    @foreach ($skillname as $skills)
                        <div class="form-check">

                            <input id="skillname" type="checkbox" name="skills[]" value="{{ $skills->id }}">
                            <label for="skillname" class="form-check-label">{{ $skills->skill_name }}</label>

                        </div>
                    @endforeach
                    <label for="photo">Project Photo</label>
                    <input type="file" id="photo" name="project_photo"/>
                    <button type="submit">Add</button>
                    <button type="button" id="closeModalBtn">cancel</button>
                    <div id="confirmationDialog" class="modal">
                        <div class="modal-content">
                            <h2>Confirmation</h2>
                            <p>Do you want to discard unsaved changes?</p>
                            <button id="cancelConfirmBtn" type="button">Cancel</button>
                            <button id="confirmChangesBtn" type="button">Confirm</button>
                        </div>
                    </div>
                </form>
            </div>
        </div>
        <!-- End New Skill  -->

        {{--===============================================--}}

        {{--    +++++++++++++++++++++++++++++++++++++++--}}

        <div class="col-lg-12">
            <div class="statbox widget box box-shadow">
                <div class="widget-content widget-content-area">
                    <table id="style-1" class="table style-1 dt-table-hover non-hover">
                        <thead>
                        <tr>
                            <th class="checkbox-column dt-no-sorting"> Record no.</th>
                            <th>Name</th>
                            <th>image</th>
                            <th>Skills</th>
                            <th>Project URL</th>
                            <th>Code URL</th>
                            <th class="text-center dt-no-sorting">Action</th>
                        </tr>
                        </thead>
                        <tbody>
                        @foreach($projects as $index=> $project)
                            <tr>
                                <td class="checkbox-column"> {{$index+1}} </td>
                                <td class="svg">{!! $project->project_name !!}</td>
                                @foreach($project->media as $media)
                                    <td><img style="width:100px;height:100px" src="{{$media->getUrl()}}"></td>
                                @endforeach
                                <td>
                                @foreach ($project->skillNames as $skill)
                                    <p>{{ $skill->skill_name }}</p>
                                @endforeach
                                </td>
                                <td><a  href="{!! $project->project_url !!}">View Project</a></td>
                                <td><a  href="{!! $project->code_url !!}">View Source Code</a></td>
                                <td class="text-center">
                                    <div class="dropdown">
                                        <a class="dropdown-toggle" role="button" id="dropdownMenuLink1"
                                           data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="true">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                 viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                                                 stroke-linecap="round" stroke-linejoin="round"
                                                 class="feather feather-more-horizontal">
                                                <circle cx="12" cy="12" r="1"></circle>
                                                <circle cx="19" cy="12" r="1"></circle>
                                                <circle cx="5" cy="12" r="1"></circle>
                                            </svg>
                                        </a>

                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuLink1">


                                            <!-- Button to open the modal for this iteration -->
                                            <a class="dropdown-item">
                                                <button style="all: unset;" class="openModalBtn2"
                                                        data-modal-target2="{{ $index }}">Update
                                                </button>
                                            </a>

                                            <form method="post" action="{{route('delete_project',['project' => $project])}}">
                                                @csrf
                                                @method('post')
                                                <a class="dropdown-item">
                                                    <button style="all: unset;" type="submit" class="dropdown-item">Delete
                                                    </button>
                                                </a>
                                            </form>
                                        </div>
                                    </div>
                                </td>
                            </tr>
                            {{--================update modal==============--}}

                            <div id="{{ 'modal2_' . $index }}" class="modal2">
                                <div class="modal-content2">
                                    <h2>Update Project</h2>

                                    <form id="{{ 'editForm2_' . $index }}" method="post"
                                          action="{{ route('update_project',['project' => $project]) }}" enctype="multipart/form-data">
                                        @csrf
                                        @method('PUT')

                                        <label for="name">Project Name</label>
                                        <input type="text" id="name" name="project_name"/>
                                        <label for="code_url">Source Code</label>
                                        <input type="text" id="code_url" name="code_url"/>
                                        <label for="project_url">Project Link</label>
                                        <input type="text" id="code_url" name="project_url"/>
                                        @foreach ($skillname as $skills)
                                            <div class="form-check">

                                                <input id="skillname" type="checkbox" name="skills[]" value="{{ $skills->id }}">
                                                <label for="skillname" class="form-check-label">{{ $skills->skill_name }}</label>
                                            </div>
                                        @endforeach
                                        <label for="photo">Project Photo</label>
                                        <input type="file" id="photo" name="project_photo"/>




                                        <button type="submit">Add</button>
                                        <button type="button" class="closeModalBtn2" data-modal-target2="{{ $index }}">
                                            Cancel
                                        </button>
                                        <div id="{{ 'confirmationDialog2_' . $index }}"
                                             class="modal2 confirmationDialog2">
                                            <div class="modal-content2">
                                                <h2>Confirmation</h2>
                                                <p>Do you want to discard unsaved changes?</p>
                                                <button class="closeModalBtn2" type="button"
                                                        data-modal-target2="{{ $index }}">Cancel
                                                </button>
                                                <button type="button" id="{{ 'confirmChangesBtn2_' . $index }}"
                                                        class="confirmChangesBtn2">Confirm
                                                </button>
                                            </div>
                                        </div>
                                    </form>
                                </div>
                            </div>
                            {{--================End update modal==============--}}
                        @endforeach
                    </tbody>
                </table>

            </div>
        </div>
    </div>
</div>
@endsection